...
const mapResponce = await axios.get(`https://nominatim.openstreetmap.org/search?q=${city}&format=json`);
if(mapResponce.data.length>0){
    const {lat,lon} = mapResponce.data[0];
    setCoords([lat,lon]);
}
